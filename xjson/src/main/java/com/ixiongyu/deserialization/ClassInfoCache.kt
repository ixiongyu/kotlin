...
import com.ixiongyu.*
import com.ixiongyu.serialization.getSerializer
...
/**
 * @author  xiongyu
 * @version Create at ：2020/10/12 8:34 下午
 */
...
class ClassInfo<T : Any>(cls: KClass<T>) {
    ...
    private val constructor = cls.primaryConstructor
            ?: throw XJsonException("Class ${cls.qualifiedName} doesn't have a primary constructor")
    ...
    private fun cacheDataForParameter(cls: KClass<*>, param: KParameter) {
        val paramName = param.name
                ?: throw XJsonException("Class $className has constructor parameter without name")
    ...
    fun getConstructorParameter(propertyName: String): KParameter = jsonNameToParamMap[propertyName]
            ?: throw XJsonException("Constructor parameter $propertyName is not found for class $className")
    ...
    private fun validateArgumentType(param: KParameter, value: Any?) {
        if (value == null && !param.type.isMarkedNullable) {
            throw XJsonException("Received null value for non-null parameter ${param.name}")
        }
        if (value != null && value.javaClass != param.type.javaType) {
            throw XJsonException("Type mismatch for parameter ${param.name}: " +
    ...
    private fun ensureAllParametersPresent(arguments: Map<KParameter, Any?>) {
        for (param in constructor.parameters) {
            if (arguments[param] == null && !param.isOptional && !param.type.isMarkedNullable) {
                throw XJsonException("Missing value for parameter ${param.name}")
            }
        }
    }
}

class XJsonException(message: String) : Exception(message)
