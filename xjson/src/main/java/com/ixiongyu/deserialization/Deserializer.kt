...
import com.ixiongyu.asJavaClass
import com.ixiongyu.isPrimitiveOrString
import com.ixiongyu.serializerForBasicType
...
fun Seed.createSeedForType(paramType: Type, isList: Boolean): Seed {
    ...
    if (List::class.java.isAssignableFrom(paramClass)) {
        if (!isList) throw XJsonException("An array expected, not a composite object")
    ...
    if (isList) throw XJsonException("Object of the type ${paramType.typeName} expected, not an array")
...
class ObjectListSeed(
    ...
    override fun setSimpleProperty(propertyName: String, value: Any?) {
        throw XJsonException("Found primitive value in collection of object types")
    }
...
class ValueListSeed(
    ...
    override fun createCompositeProperty(propertyName: String, isList: Boolean): Seed {
        throw XJsonException("Found object value in collection of primitive types")
    }
